...
class SportsTests(unittest.TestCase):
    ...
    def test_om_filmes(self):
        news, title = nse.get_most_read('en_om_filmes')
        self.assertEqual(len(news), 5)

    def test_om_series(self):
        news, title = nse.get_most_read('en_om_series')
        self.assertEqual(len(news), 5)

    def test_om_hqs(self):
        news, title = nse.get_most_read('en_om_hqs')
        self.assertEqual(len(news), 5)
